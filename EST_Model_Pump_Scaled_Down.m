...
rho_w = 997;
visc_w = 8.9e-4; %Dynamic viscosity seawater [Pa*s]
r = 0.015; %Relative roughness
n_pump = 0.25; %Efficiency

%% Test Parameters
D_pipe = 0.006; %Diameter of pipe/hydrogenerator [m]
    A_pipe = pi*(D_pipe^2)/4; %Cross-sectional area pipe [m^2]
N_pipe = 1; %Number of pipes/generators
L_pipe = 0.3; %Length of the pipe [m]
% L_reservoir = 3000; %Length/width reservoir [m]
    A_reservoir = 0.5^2*pi/4; %Cross-sectional area reservoir [m^2]
...
H_sea = 2; %Sea level height [m]

P_surplus = 2.4; %Power Surplus [W]
